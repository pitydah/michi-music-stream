...
CANONICAL_NOTO_REGULAR_SHA256 = "89c3c497f618fdaa0b2d1e98fef93582f28c71debd2c4a8cdf41f190ced2909d"
CANONICAL_NOTO_MEDIUM_SHA256 = "013ae62f5187c01e8752de78829ee7e1e0feaed6edee17db0dbf703988c99f3d"
...
def emit_fonts(font_family="notosans", raster_bits_override=1,
               regular_path=None, medium_path=None, output_dir=None, strict=False):
    fonts_out = os.path.join(output_dir, "fonts", "michi_ui_fonts_data.h") if output_dir else FONTS_OUT
    if font_family == "inter":
        inter_reg = regular_path or _find_inter("Regular")
        inter_med = medium_path or _find_inter("Medium")
        if inter_reg is None:
            print("WARNING: Inter Regular not found; falling back to Noto Sans Regular.", file=sys.stderr)
            inter_reg = _find_noto("Regular")
        if inter_med is None:
            print("WARNING: Inter Medium not found; falling back to Noto Sans Medium.", file=sys.stderr)
            inter_med = _find_noto("Medium")
        font_regular = inter_reg
        font_medium  = inter_med
        family_label = "Inter"
    else:
        font_regular = regular_path or _find_noto("Regular")
        font_medium  = medium_path or _find_noto("Medium")
        if font_regular is None:
            raise FileNotFoundError("Noto Sans Regular font not found in search paths")
        if strict or os.environ.get("MICHI_STRICT_ASSETS") == "1":
            if font_medium is None:
                raise FileNotFoundError("Noto Sans Medium font not found (strict mode: fallback disabled)")
            reg_sha = sha256_file(font_regular)
            med_sha = sha256_file(font_medium)
            if reg_sha != CANONICAL_NOTO_REGULAR_SHA256:
                raise ValueError(
                    f"Canonical Noto Sans Regular SHA-256 mismatch!\n"
                    f"  File:     {font_regular}\n"
                    f"  Expected: {CANONICAL_NOTO_REGULAR_SHA256}\n"
                    f"  Actual:   {reg_sha}"
                )
            if med_sha != CANONICAL_NOTO_MEDIUM_SHA256:
                raise ValueError(
                    f"Canonical Noto Sans Medium SHA-256 mismatch!\n"
                    f"  File:     {font_medium}\n"
                    f"  Expected: {CANONICAL_NOTO_MEDIUM_SHA256}\n"
                    f"  Actual:   {med_sha}"
                )
        else:
            if font_medium is None:
    ...
    reg_size = os.path.getsize(font_regular)
    med_size = os.path.getsize(font_medium)
    reg_hash = sha256_file(font_regular)
    med_hash = sha256_file(font_medium)
    print(f"[michi-ui-assets] Pillow version: {PIL_VER}")
    print(f"[michi-ui-assets] Regular font:   {font_regular} ({reg_size} bytes, sha256: {reg_hash})")
    print(f"[michi-ui-assets] Medium font:    {font_medium} ({med_size} bytes, sha256: {med_hash})")
...
def emit_icons(output_dir=None):
    icons_out = os.path.join(output_dir, "icons", "michi_ui_icons_data.h") if output_dir else ICONS_OUT
    lines = []
...
def main():
    ...
    parser.add_argument(
        "--regular-font", metavar="PATH", default=None,
        help="Explicit path to Regular font TTF file.")
    parser.add_argument(
        "--medium-font", metavar="PATH", default=None,
        help="Explicit path to Medium font TTF file.")
    parser.add_argument(
        "--output-dir", metavar="DIR", default=None,
        help="Directory to place generated assets (defaults to assets/).")
    parser.add_argument(
        "--strict", action="store_true",
        help="Enforce strict canonical font SHA-256 verification and disable fallbacks.")
    parser.add_argument(
    ...
    emit_fonts(
        ...
        raster_bits_override=raster_bits,
        regular_path=args.regular_font,
        medium_path=args.medium_font,
        output_dir=args.output_dir,
        strict=args.strict,
    )
    emit_icons(output_dir=args.output_dir)
