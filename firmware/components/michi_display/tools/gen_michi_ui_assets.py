...
def emit_fonts(font_family="notosans", raster_bits=1):
    """Emit the C font data header.

    Args:
        font_family: "notosans" or "inter" – selects which TTF files to use.
        raster_bits: 1 or 2 – 1-bit on/off bitmaps or 2-bit coverage bitmaps.
    """
    # Resolve actual TTF paths.
    ...
    def _pil_glyph_2bit(font, ch, box_h):
        """Render glyph with 2-bit (4-level) coverage per pixel column."""
        tmp = Image.new("L", (800, box_h + 8))
        d = ImageDraw.Draw(tmp)
        d.text((0, 0), ch, fill=255, font=font)
        bbox = d.textbbox((0, 0), ch, font=font)
        ...
        # Map 8-bit greyscale to 2-bit (0-3) coverage per pixel.
        cols = []
        for x in range(w):
            col = 0
            for y in range(box_h):
                pix = glyph.getpixel((x, y))
                level = min(3, pix >> 6)  # 0-3
                col |= level << (2 * y)
            cols.append(col)
        return cols, w, adv

    if raster_bits == 2:
        glyph_fn = _pil_glyph_2bit
        bpc_multiplier = 2  # 2 bits per row → 2× bytes per column
    else:
        glyph_fn = pil_glyph
        bpc_multiplier = 1
    ...
    def emit_font(tag, glyphs, height, baseline, line_h):
        if raster_bits == 2:
            bpc = -(-height // 4)  # ceil(height * 2 bits / 8)
        else:
            bpc = -(-height // 8)  # ceil(height / 8)
        bitmap = bytearray()
        width = []
        advance = []
        offset = []
        ...
        lines.append(f"/* michi_ui_{tag}: height={height} baseline={baseline} line_height={line_h} "
                     f"bytes_per_col={bpc} raster={raster_label} */\n")

    # XS (pt 6 thresholded – always 1-bit for crispness at 8px)
    ...
    for tag, (path, pt, box_h, line_h) in PIL_FONTS.items():
        ...
        glyphs = {ch: glyph_fn(font, ch, box_h) for ch in FULL_CHARS}
        emit_font(tag, glyphs, box_h, asc, line_h)

    # PIN (charset subset + pin_map) – always 1-bit for digit clarity
    _, pt, box_h, line_h = PIL_FONTS["pin"]
    font = ImageFont.truetype(FONT_REGULAR, pt)
    asc, _ = font.getmetrics()
    pin_glyphs = {ch: pil_glyph(font, ch, box_h) for ch in FULL_CHARS}

    # Build pin_map (full-charset index → pin glyph index)
    pin_map = []
    for ch in FULL_CHARS:
        if ch in PIN_CHARS:
    ...
    emit_font("pin", pin_glyphs, box_h, asc, line_h)
    ...
    with open(FONTS_OUT, "w") as f:
        f.writelines(lines)
    print(f"wrote {FONTS_OUT} (font={family_label}, raster={raster_label})")
...
def main():
    ...
    parser.add_argument(
        "--font", choices=["notosans", "inter"], default="inter",
        help="Font family to rasterize (default: inter).")
    parser.add_argument(
        "--raster", choices=["1bit", "2bit"], default="2bit",
        help="Bitmap depth: 1bit = on/off, 2bit = 4-level AA (default: 2bit).")
    args = parser.parse_args()

    raster_bits = 2 if args.raster == "2bit" else 1
    emit_fonts(font_family=args.font, raster_bits=raster_bits)
    emit_icons()
